...
Um in der Pipeline eine konsistente Testumgebung sicherzustellen, wurde ein separates Initialisierungsskript \texttt{init-ldap.sh} erstellt. Dieses Skript stellt sicher, dass der OpenLDAP-Container vollständig gestartet ist und initialisiert anschlieend die Datenbank mit einer vordefinierten Struktur sowie Testdaten. Dazu gehören die benutzerdefinierte Schema-Datei, die organisatorische Struktur, Gruppen und Beispielbenutzer.
...
\begin{enumerate}
    ...
    \item \textbf{Verfügbarkeit prüfen:} Es wird mit einem einfachen \texttt{curl}-Befehl regelmäig geprüft, ob die Applikation über ihren Heartbeat-Endpunkt erreichbar ist.
    ...
    \item \textbf{HTML Report:} Die ausgeführten API-Tests werden alls HTML Report gespeichert.
